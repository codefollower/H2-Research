...
public class ValueLobTest {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 1; i++) {
			...
			String fileName = getFileNamePrefix(path, objectId)
	...
	static String getFileNamePrefix(String path, int objectId) {
		...
		name = FileUtils.toRealPath(path + Constants.SUFFIX_LOBS_DIRECTORY
				+ name);
		return name;
	}
	...
	static String[] getFileList(DataHandler h, String dir) {
		...
		if (cache == null) {
			...
			synchronized (cache) {
				list = cache.get(dir);
				if (list == null) {
					list = FileUtils.newDirectoryStream(dir).toArray(
							new String[0]);
					cache.put(dir, list);
				}
			}
