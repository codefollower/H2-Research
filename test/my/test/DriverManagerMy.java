 ...
 * <B>NOTE:</B> The {@link <code>DataSource</code>} interface, new in the JDBC
 * 2.0 API, provides another way to connect to a data source. The use of a
 * <code>DataSource</code> object is the preferred means of connecting to a data
 * source.
 * 
 * <P>
 * As part of its initialization, the <code>DriverManagerMy</code> class will
 ...
 * </pre>
 * <P>
 * The <code>DriverManagerMy</code> methods <code>getConnection</code> and
 * <code>getDrivers</code> have been enhanced to support the Java Standard
 * Edition <a
 * href="../../../technotes/guides/jar/jar.html#Service%20Provider">Service
 ...
 * <code>DriverManagerMy</code> will attempt to locate a suitable driver from
...
public class DriverManagerMy {
     ...
     * the <code>DriverManagerMy</code> and all drivers.
     * <P>
     ...
     * <code>DriverManagerMy</code> attempts to select an appropriate driver
    ...
    public static Connection getConnection(String url, java.util.Properties info) throws SQLException {
        ...
        // be null.
        ClassLoader callerCL = DriverManagerMy.getCallerClassLoader();
    ...
    public static Driver getDriver(String url) throws SQLException {
        ...
        println("DriverManagerMy.getDriver(\"" + url + "\")");

        if (!initialized) {
            initialize();
        }

        synchronized (DriverManagerMy.class) {
    ...
    /**
     * Drops a driver from the <code>DriverManagerMy</code>'s list. Applets can
    ...
    public static synchronized void deregisterDriver(Driver driver) throws SQLException {
        ...
        // may be null.
        ClassLoader callerCL = DriverManagerMy.getCallerClassLoader();
        println("DriverManagerMy.deregisterDriver: " + driver);
     ...
     * <code>DriverManagerMy</code> and all drivers.
     * <P>
    ...
    private static void loadInitialDrivers() {
        ...
        try {
            drivers = (String) java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction(
                    "jdbc.drivers"));
        } catch (Exception ex) {
            drivers = null;
        }
        ...
        println("DriverManagerMy.initialize: jdbc.drivers = " + drivers);
        if (drivers == null) {
            return;
        }
        while (drivers.length() != 0) {
            ...
            try {
                println("DriverManagerMy.Initialize: loading " + driver);
                Class.forName(driver, true, ClassLoader.getSystemClassLoader());
            } catch (Exception ex) {
                println("DriverManagerMy.Initialize: load failed: " + ex);
            }
        }
    }
    ...
    private static Connection getConnection(String url, java.util.Properties info, ClassLoader callerCL) throws SQLException {
        ...
        println("DriverManagerMy.getConnection(\"" + url + "\")");
    ...
    static void initialize() {
        ...
        initialized = true;
        loadInitialDrivers();
        println("JDBC DriverManagerMy initialized");
    }

    /* Prevent the DriverManagerMy class from being instantiated. */
    private DriverManagerMy() {
    }
...
class DriverService implements java.security.PrivilegedAction {
    ...
    public Object run() {
         ...
         * java.sql.Driver but the actual class may be missing. In that case a
         * sun.misc.ServiceConfigurationError will be thrown at runtime by the
         * VM trying to locate and load the service.
         * 
